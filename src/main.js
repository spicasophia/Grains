...
async function paymentApproval(_price) {
  ...
  const result = await ERCContract.methods
    .approve(GrainContractAddress, _price)
    .send({
  ...
  for (let index = 0; index < hiresLength; index++) {
    let _hire = new Promise(async (resolve, reject) => {
      await contract.methods
        .getServiceHire(serviceIndex, index)
        .call()
        .then((h) => {
          resolve({
            index: index,
...
const getServices = async function () {
  ...
  document.querySelector("#labourersCounts").textContent =
    _servicesLength + " labourers";
...
document
  ...
  .addEventListener("click", async (e) => {
    ...
    let productPrice = new BigNumber(newPrice)
      .shiftedBy(ERC20_DECIMALS)
      .toString();
    let productFee = new BigNumber(0.05 * newPrice)
      .shiftedBy(ERC20_DECIMALS)
      .toString();
...
document
  ...
  .addEventListener("click", async (e) => {
    const serviceParams = [
      ...
      new BigNumber(document.getElementById("newServiceRate").value)
        .shiftedBy(ERC20_DECIMALS)
        .toString(),
    ];
    ...
    try {
      await contract.methods
        .addService(...serviceParams)
        .send({
          from: kit.defaultAccount,
        })
        .then(() => {
          serviceNotification(
            `🎉 You successfully added "${serviceParams[0]}".`
          );
          getServices();
        })
        .catch((err) => {
...
async function renderServiceHires(index, hires) {
  ...
  if (hires) {
    ...
    document.getElementById(hireID).innerHTML =
      "<p class='text-center'>No Service Hire</p>";
  }
}
...
<a class="btn btn-lg btn-outline-dark buyBtn fs-6 p-3" id=${_product.index}>
    Buy for ${_product.price
      .shiftedBy(-ERC20_DECIMALS)
      .toFixed(2)}(${_product.fee.shiftedBy(-ERC20_DECIMALS).toFixed(2)}) cUSD
  </a>
</div>`;
  return `
    <div class="card mb-4">
      <img class="card-img-top" style="max-height:100%" src="${
        _product.image
      }" alt="...">
      ...
      <div class="card-body text-left p-4 position-relative">
        ...
        ${_product.owner == kit.defaultAccount ? "" : buyBtn}
      </div>
    </div>
  `;
}
function serviceTemplate(_service) {
  ...
  const hireBtn = `<div class="d-grid gap-2">
  <a class="btn btn-lg btn-outline-dark hireBtn fs-6 p-3" id=${_service.index}>
  ...
  return `
    <div class="card mb-4">
      ...
      <div class="position-absolute top-0 end-0 bg-warning mt-4 px-2 py-1 rounded-start">
        <button type="button" class="btn btn-link" data-bs-toggle="modal" data-bs-target="#service${
          _service.index
        }Modal" style="text-decoration: none;">
      ...
      <div class="card-body text-dark text-left p-4 position-relative">
        ...
        ${_service.user == kit.defaultAccount ? "" : hireBtn}
    ...
    <div class="modal fade" id="service${
      _service.index
    }Modal" tabindex="-1" aria-labelledby="service${
    _service.index
  }ModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content text-dark">
          <div class="modal-header">
            <h5 class="modal-title" id="service${_service.index}ModalLabel">${
    _service.name
  } Hires</h5>
...
document
  .querySelector("#Grainmarketplace")
  .addEventListener("click", async (e) => {
    if (e.target.className.includes("buyBtn")) {
      ...
      try {
        let bigSum =
          BigInt(products[index].price) + BigInt(products[index].fee);
...
document
  ...
  .addEventListener("click", async (e) => {
    if (e.target.className.includes("hireBtn")) {
      ...
      serviceNotification(
        `⌛ Awaiting payment to hire "${services[index].name}"...`
      );
      try {
        await contract.methods
          .hireService(index, servicePrice, services[index].user)
          .send({
            from: kit.defaultAccount,
          });
        serviceNotification(
          `🎉 You successfully hired "${services[index].name}".`
        );
        getBalance();
        getServices();
      } catch (error) {
